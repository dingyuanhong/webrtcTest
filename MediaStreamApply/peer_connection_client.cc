...
void PeerConnectionClient::RegisterObserver(
	PeerConnectionTransformObserver* callback) {
  ASSERT(!callback_);
  callback_ = callback;
}
...
void PeerConnectionClient::DoConnect() {
  ...
  {
	 callback_->OnServerConnectionSuccess();
  }
}
...
void PeerConnectionClient::Close() {
  if(control_socket_.get()) control_socket_->Close();
  if (hanging_get_.get()) hanging_get_->Close();
